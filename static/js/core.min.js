 ...
 * @module  О±Util
 * @version v1.0.1
 * @desc    Service functions for О±-modules
 ...
 * @license  CC BY-SA 4.0 {@link https://creativecommons.org/licenses/by-sa/4.0/}
 * @requires module:О±Util
 * @requires module:О±ProgressCircle
 * @see      {@link https://github.com/OXAYAZA/Alpha-Counter}
 */
 ...
 */
function simpleFilter(t) { function i(t) { t.filterGroup = this, this.node = t, this.filters = t.querySelectorAll("[data-filter]"), this.items = t.querySelectorAll("[data-category]"), this.current = "*", this.init() } return i.prototype.init = function () { for (var t = 0; t < this.filters.length; t++) { var i = this.filters[t]; i.filterGroup = this, i.addEventListener("click", this.clickHandler) } for (t = 0; t < this.items.length; t++)this.items[t].filterGroup = this; this.node.addEventListener("filter", this.filterHandler.bind(this)) }, i.prototype.clickHandler = function () { for (let t = 0; t < this.filterGroup.filters.length; t++)this.filterGroup.filters[t].classList.remove("active"); this.filterGroup.filter(this.getAttribute("data-filter")), this.classList.add("active") }, i.prototype.filter = function (t) { var i = new CustomEvent("filter"); i.filterName = t, this.current = t, this.node.dispatchEvent(i) }, i.prototype.filterHandler = function () { for (var t = 0; t < this.items.length; t++) { var i = this.items[t]; this.current === i.getAttribute("data-category") || "*" === this.current ? i.classList.remove("hidden") : i.classList.add("hidden") } }, new i(t) }
 ...
 */
var mul_table = [512, 512, 456, 512, 328, 456, 335, 512, 405, 328, 271, 456, 388, 335, 292, 512, 454, 405, 364, 328, 298, 271, 496, 456, 420, 388, 360, 335, 312, 292, 273, 512, 482, 454, 428, 405, 383, 364, 345, 328, 312, 298, 284, 271, 259, 496, 475, 456, 437, 420, 404, 388, 374, 360, 347, 335, 323, 312, 302, 292, 282, 273, 265, 512, 497, 482, 468, 454, 441, 428, 417, 405, 394, 383, 373, 364, 354, 345, 337, 328, 320, 312, 305, 298, 291, 284, 278, 271, 265, 259, 507, 496, 485, 475, 465, 456, 446, 437, 428, 420, 412, 404, 396, 388, 381, 374, 367, 360, 354, 347, 341, 335, 329, 323, 318, 312, 307, 302, 297, 292, 287, 282, 278, 273, 269, 265, 261, 512, 505, 497, 489, 482, 475, 468, 461, 454, 447, 441, 435, 428, 422, 417, 411, 405, 399, 394, 389, 383, 378, 373, 368, 364, 359, 354, 350, 345, 341, 337, 332, 328, 324, 320, 316, 312, 309, 305, 301, 298, 294, 291, 287, 284, 281, 278, 274, 271, 268, 265, 262, 259, 257, 507, 501, 496, 491, 485, 480, 475, 470, 465, 460, 456, 451, 446, 442, 437, 433, 428, 424, 420, 416, 412, 408, 404, 400, 396, 392, 388, 385, 381, 377, 374, 370, 367, 363, 360, 357, 354, 350, 347, 344, 341, 338, 335, 332, 329, 326, 323, 320, 318, 315, 312, 310, 307, 304, 302, 299, 297, 294, 292, 289, 287, 285, 282, 280, 278, 275, 273, 271, 269, 267, 265, 263, 261, 259], shg_table = [9, 11, 12, 13, 13, 14, 14, 15, 15, 15, 15, 16, 16, 16, 16, 17, 17, 17, 17, 17, 17, 17, 18, 18, 18, 18, 18, 18, 18, 18, 18, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 22, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24]; function customBlur(t, a, r) { var e = t.naturalWidth, n = t.naturalHeight; a.style.width = e + "px", a.style.height = n + "px", a.width = e, a.height = n; var l = a.getContext("2d"); l.clearRect(0, 0, e, n), l.drawImage(t, 0, 0), isNaN(r) || r < 1 || stackBlurCanvasRGB(a, 0, 0, e, n, r) } function stackBlurCanvasRGB(t, a, r, e, n, l) { if (!(isNaN(l) || l < 1)) { l |= 0; var g, i = t.getContext("2d"); try { g = i.getImageData(a, r, e, n) } catch (t) { throw new Error("unable to access image data: " + t) } var u, o, s, h, b, c, x, f, v, d, B, m, w, k, C, N, _, p, y, I, R = g.data, S = l + l + 1, D = e - 1, G = n - 1, E = l + 1, H = E * (E + 1) / 2, W = new BlurStack, j = W; for (s = 1; s < S; s++)if (j = j.next = new BlurStack, s == E) var q = j; j.next = W; var z = null, A = null; x = c = 0; var F = mul_table[l], J = shg_table[l]; for (o = 0; o < n; o++) { for (k = C = N = f = v = d = 0, B = E * (_ = R[c]), m = E * (p = R[c + 1]), w = E * (y = R[c + 2]), f += H * _, v += H * p, d += H * y, j = W, s = 0; s < E; s++)j.r = _, j.g = p, j.b = y, j = j.next; for (s = 1; s < E; s++)h = c + ((D < s ? D : s) << 2), f += (j.r = _ = R[h]) * (I = E - s), v += (j.g = p = R[h + 1]) * I, d += (j.b = y = R[h + 2]) * I, k += _, C += p, N += y, j = j.next; for (z = W, A = q, u = 0; u < e; u++)R[c] = f * F >> J, R[c + 1] = v * F >> J, R[c + 2] = d * F >> J, f -= B, v -= m, d -= w, B -= z.r, m -= z.g, w -= z.b, h = x + ((h = u + l + 1) < D ? h : D) << 2, f += k += z.r = R[h], v += C += z.g = R[h + 1], d += N += z.b = R[h + 2], z = z.next, B += _ = A.r, m += p = A.g, w += y = A.b, k -= _, C -= p, N -= y, A = A.next, c += 4; x += e } for (u = 0; u < e; u++) { for (C = N = k = v = d = f = 0, B = E * (_ = R[c = u << 2]), m = E * (p = R[c + 1]), w = E * (y = R[c + 2]), f += H * _, v += H * p, d += H * y, j = W, s = 0; s < E; s++)j.r = _, j.g = p, j.b = y, j = j.next; for (b = e, s = 1; s <= l; s++)c = b + u << 2, f += (j.r = _ = R[c]) * (I = E - s), v += (j.g = p = R[c + 1]) * I, d += (j.b = y = R[c + 2]) * I, k += _, C += p, N += y, j = j.next, s < G && (b += e); for (c = u, z = W, A = q, o = 0; o < n; o++)R[h = c << 2] = f * F >> J, R[h + 1] = v * F >> J, R[h + 2] = d * F >> J, f -= B, v -= m, d -= w, B -= z.r, m -= z.g, w -= z.b, h = u + ((h = o + E) < G ? h : G) * e << 2, f += k += z.r = R[h], v += C += z.g = R[h + 1], d += N += z.b = R[h + 2], z = z.next, B += _ = A.r, m += p = A.g, w += y = A.b, k -= _, C -= p, N -= y, A = A.next, c += e } i.putImageData(g, a, r) } } function BlurStack() { this.r = 0, this.g = 0, this.b = 0, this.a = 0, this.next = null }
